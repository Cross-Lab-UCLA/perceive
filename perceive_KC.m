                ...
                case 'BrainSenseTimeDomain'
                    ...
                    for c = 1:length(runs)
                        ...
                        try 
                            ...
                            if ~isempty(x)
                                 warning('missing packages detected, missing data replaced with nan')
                                
                                % RECONSTRUCT TIME TO IDENTIFY DROPPED PACKETS BASED ON MEDTRONIC WHITE PAPER METHOD
                                % assuming same packets for both channels
                                for k=1:numel(x)-1
                                    assert(sum(GlobalPacketSizes{i(k),:} - GlobalPacketSizes{i(k+1),:}) == 0 ,'Sample size differed between channels. Check session affiliation.');
                                end
                                k=1;
                                nPackets = numel(GlobalPacketSizes{i(k),:});
                                          
                                % last message time array
                                t = TicksInS{i(k),:}(end) - (GlobalPacketSizes{i(k),:}(end)-1) ./ fsample: 1/fsample : TicksInS{i(k),:}(end);
                                
                                for packetId = nPackets:-1:2
                                
                                    if TicksInS{i(k),:}(packetId) - TicksInS{i(k),:}(packetId-1) > (1+GlobalPacketSizes{i(k),:}(packetId)) ./ fsample
                                        prevPacketTime = TicksInS{i(k),:}(packetId-1) - (GlobalPacketSizes{i(k),:}(packetId-1) - 1) ./ fsample : 1./ fsample : TicksInS{i(k),:}(packetId-1);
                                    else
                                        prevPacketTime = t(1) - GlobalPacketSizes{i(k),:}(packetId-1) ./ 1/fsample : 1/fsample : t(1) - 1/fsample;
                                    end
                                    t = [prevPacketTime, t];
                                end
                        ...
                        %         raw=raw_temp;
                                ...
                                raw=[data(i).TimeDomainData]';
                                t = linspace(0,size(raw,2)/fsample,size(raw,2));
                            end
                        ...
                        d.time{1} = seconds(datetime(runs{c},'Inputformat','yyyy-MM-dd HH:mm:ss.sss')-hdr.d0) + t; 
