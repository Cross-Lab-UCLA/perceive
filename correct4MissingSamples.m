...
time = TicksInS(end) - (GlobalPacketSizes(end)-1) ./ LFP.Fs: 1/LFP.Fs : TicksInS(end);

for packetId = nPackets:-1:2

    if TicksInS(packetId) - TicksInS(packetId-1) > (1+GlobalPacketSizes(packetId)) ./ LFP.Fs
        prevPacketTime = TicksInS(packetId-1) - (GlobalPacketSizes(packetId-1) - 1) ./ LFP.Fs : 1./ LFP.Fs : TicksInS(packetId-1);
    else
        prevPacketTime = time(1) - GlobalPacketSizes(packetId-1) ./ 1/LFP.Fs : 1/LFP.Fs : time(1) - 1/LFP.Fs;
    end
    time = [prevPacketTime, time];
end
